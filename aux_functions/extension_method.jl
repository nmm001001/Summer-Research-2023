...
All bar extensions are computed under the default interval decomposition of ``barcode(C_Y)`` used by Eirene_var. 
...
Create a dictionary with key: parameter and values: baseline, offset bar-extensions. Note that all bar extensions are computed using the default interval decomposition of the target barcode used by Eirene_var.
...
function plot_analogous_bars(extension_P, extension_Q; 
    ...
    title = plot(title = "test", grid = false, showaxis = false, bottom_margin = -50Plots.px)
...
#Niko added the following to plot analogous bars with just one of the witness complexes shown
"""
    plot_analogous_bars_one_witness_complex()
Plots summary of the analogous bars method.
1. plots 4 barcodes: Barcode(VR(P)), Barcode(W(P,Q)), Barcode(W(Q,P)), Barcode(VR(Q)).
2. Highlights the selected Witness bar in Barcode(W(P,Q)) and Barcode(W(Q,P))
3. Highlights the **BASELINE** bar extension to Barcode(VR(P)) and Barcode(VR(Q)) at **epsilon_0**
"""
function plot_analogous_bars_one_witness_complex(extension_P, extension_Q;
                            trial_no = nothing,
                            titlefontsize = 10,
                            lw_VR_P = nothing,
                            lw_W = nothing,
    ...
    # plot
    p_VR_P = plot_barcode(barcode_VR_P,
                            selected_bars = bar_ext_P, title = title_VR_P, top_margin = 50Plots.px,
                            titlefontsize = titlefontsize,
                            lw = lw_VR_P,
                            epsilon = epsilon0_P,
                            v_line = v_line_P;
    ...
    p_W_PQ = plot_barcode(barcode_W_PQ,
                            selected_bars = extension_P["selected_bar"], 
                            selection_color = :deepskyblue2,
                            title = title_W_PQ, top_margin = 50Plots.px,
                            titlefontsize = titlefontsize,
                            lw = lw_W;
                            kwargs...)
    p_W_QP = plot_barcode(barcode_W_QP,
                            selected_bars = extension_Q["selected_bar"], 
                            selection_color = :deepskyblue2,
                            title = title_W_QP, top_margin = 50Plots.px,
                            titlefontsize = titlefontsize,
                            lw = lw_W;
                            kwargs...)
    p_VR_Q = plot_barcode(barcode_VR_Q,
                            selected_bars = bar_ext_Q, 
                            title = title_VR_Q, top_margin = 50Plots.px,
                            titlefontsize = titlefontsize,
                            lw = lw_VR_Q,
                            epsilon = epsilon0_Q,
                            v_line = [epsilon0_Q];
                            kwargs...)
    if trial_no == nothing
        title = plot(title = "Analogous bars", grid = false, showaxis = false, bottom_margin = -10Plots.px)
    else
        title = plot(title = "Analogous bars trial $trial_no", grid = false, showaxis = false, bottom_margin = -10Plots.px)
    end
    p = plot(title, p_VR_P, p_W_PQ, p_VR_Q, layout = @layout([A{0.08h}; [B; C; D]]), size = (500, 1000))
    return p
end


function plot_analogous_bars_one_witness_complex_with_auxiliary(extension_P, extension_Q;
                            title = nothing,
                            titlefontsize = 10,
                            lw_VR_P = nothing,
                            lw_W = nothing,
                            lw_VR_Q = nothing,
                            title_VR_P = "Baseline bar extension in Barcode(VR(P))",
                            title_P_aux = "Auxiliary filtration P",
                            title_W_PQ = "Selected bar in Barcode(W(P,Q))",
                            title_W_QP = "Selected bar in Barcode(W(Q,P))",
                            title_Q_aux = "Auxiliary filtration Q",
                            title_VR_Q = "Baseline bar extension in Barcode(VR(Q))",
                            kwargs...)
    ...
    d = extension_P["dim"]   
    barcode_VR_P = barcode(extension_P["C_VR"], dim = d)
    barcode_P_aux = barcode(extension_P["C_auxiliary_filtration"])
    barcode_W_PQ = barcode(extension_P["C_W"], dim = d)
    barcode_W_QP = barcode(extension_Q["C_W"], dim = d)
    barcode_Q_aux = barcode(extension_Q["C_auxiliary_filtration"])
    barcode_VR_Q = barcode(extension_Q["C_VR"], dim = d)
    ...
    p_VR_P = plot_barcode(barcode_VR_P,
                            selected_bars = bar_ext_P, title = title_VR_P, top_margin = 50Plots.px,
                            titlefontsize = titlefontsize, xlims=(-0.05, 1.05),
                            lw = lw_VR_P,
                            epsilon = epsilon0_P,
                            v_line = v_line_P;
                            kwargs...)
    p_P_aux = plot_barcode(barcode_P_aux,
                            selected_bars = keys(extension_P["aux_filt_cyclerep"]), 
                            selection_color = :deepskyblue2,
                            title = title_P_aux, top_margin = 50Plots.px,
                            titlefontsize = titlefontsize,
                            lw = lw_W;
                            kwargs...)
    p_W_PQ = plot_barcode(barcode_W_PQ,
    ...
    p_W_QP = plot_barcode(barcode_W_QP,
                            ...
                            lw = lw_W;
                            kwargs...)
    p_Q_aux = plot_barcode(barcode_Q_aux,
                        selected_bars = keys(extension_Q["aux_filt_cyclerep"]), 
                        selection_color = :deepskyblue2,
                        title = title_Q_aux, top_margin = 50Plots.px,
                        titlefontsize = titlefontsize,
                        lw = lw_W;
                        kwargs...)
    p_VR_Q = plot_barcode(barcode_VR_Q,
    ...
    if title == nothing
        ...
        title = plot(title = title, grid = false, showaxis = false, bottom_margin = -10Plots.px)
    end
    p = plot(title, p_VR_P, p_P_aux, p_W_PQ, p_Q_aux, p_VR_Q, layout = (3,2), size = (500, 1000))
    return p
end
...
function get_vertex_perm(C::Dict)
    ...
    # Get vertex permutation information from Eirene_var.
...
function check_homologous_cycles(
    ...
    w_chain: (array) of indices of simplices of [w].
            Note that simplex indexing must be consistent with C. 
    C: (dict) output of Eirene_var. When running, eirene, must use the argument `record = "all"`
    --- output ---
...
function find_cyclereps_auxiliary_WtoVR(Fbar_representation_tau, BARS_short, C_aux, Wpsi_index2simplex_default_vertex; dim = 1)
    ...
    # #Niko added:
    # for bar in Fbar_representation_tau
    #     cyclerep_chain = classrep(C_aux, dim = dim, class = bar)  
    #     cyclerep_v = [Wpsi_index2simplex_default_vertex[(item, dim)] for item in cyclerep_chain]
    #     aux_cycles[bar] = cyclerep_v
    # end
...
function chain_to_index(
    ...
    # where each $i_j$ is the index of simplex_j in C_n according to Eirene_var. 
