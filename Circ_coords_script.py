...
file = open("Trial folders/date.txt")
date_now = file.read()
file.close()
...
def import_distance_matrices(D_x):
    ...
    D_x = (h5py.File(f"{D_x}", "r")).get("distance")
...
# D_P = convert_Dx_to_array(import_distance_matrices("Trial folders/{date_now}/h5 files/distance_dp.h5"))
# D_Q = convert_Dx_to_array(import_distance_matrices("Trial folders/{date_now}/h5 files/distance_dq.h5"))
...
def gen_toroidal_coords(D_x, num_landmarks=50, percent=0.1, cohomology_classes=[0], plot_tc=False):
    ...
    if plot_tc:
        ...
        plt.savefig(f"output_pop_size_50/trial_1/barcode/{D_x}_tc_neuron_plot.png")
        plt.close()
...
def plt_tc_neurons(toroidal_coords, D_x):
    ...
    plt.savefig(f"output_pop_size_50/trial_1/barcode/{D_x}_tc_neurons.png")
    plt.close()
    return
...
def plt_tc_on_circle(toroidal_coords, file_name):
    ...
    plt.scatter(xs,ys)
    plt.savefig(f"output_pop_size_50/trial_1/barcode/{file_name}_tc_circular.png")
    plt.close()
    return



D_P = convert_Dx_to_array(import_distance_matrices(f"output_pop_size_50/trial_1/barcode/distance_dp.h5"))
D_Q = convert_Dx_to_array(import_distance_matrices(f"output_pop_size_50/trial_1/barcode/distance_dq_50.h5"))
